...
create or replace procedure proc(var1 in dept.dept_name%type)
as

cursor c is select * from course where dept_id=(select dept_id from dept where dept_name=var1);
...
fetch c into course_name;
while c%found loop

dbms_output.put_line('Course: ' || course_row.course_name);fetch c into course_name;
end loop;
close course_cursor;
end;
/
